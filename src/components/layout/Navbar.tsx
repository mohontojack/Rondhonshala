...
export function Navbar({ activeTab, setActiveTab }: NavbarProps) {
  ...
  const navItems = [
    ...
    { id: 'recipes', label: 'রেসিপি', icon: UtensilsCrossed },
    { id: 'blog', label: 'ব্লগ', icon: ImageIcon },
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="bg-[#FCFAF7] text-primary text-[9px] font-black uppercase tracking-[0.4em] py-2.5 px-4 text-center border-b border-charcoal/5 relative z-[60]">
        <span className="opacity-70">Exclusivity: Pre-order 7 days in advance</span>
        <span className="mx-4 text-charcoal/10">|</span>
        <span className="text-charcoal font-black">Friday & Saturday Delivery in Dinajpur Only</span>
      </div>
      <nav className="sticky top-0 z-50 glass border-b border-charcoal/5 transition-all duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-24 items-center">
          <div className="flex items-center gap-5 cursor-pointer group" onClick={() => setActiveTab('home')}>
            <Logo className="h-16 w-16 group-hover:scale-110 transition-transform" />
            <div className="hidden sm:block">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] uppercase tracking-[0.5em] text-primary font-black opacity-60">Dinajpur's Finest</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-8">
            <div className="hidden xl:flex flex-col items-end">
              <span className="text-[9px] uppercase font-black text-charcoal/30 tracking-[0.3em] opacity-40 mb-1 leading-none">Inquiry Hotline</span>
            ...
            <button 
              ...
              className={cn(
                ...
                activeTab === 'admin' ? "bg-primary text-white border-primary shadow-premium" : "bg-white border-charcoal/5 text-charcoal/20 hover:text-primary hover:border-primary/40 shadow-sm"
              )}
            >
            ...
            <div className="md:hidden">
              <button onClick={() => setIsOpen(!isOpen)} className="text-primary hover:bg-primary/5 p-4 rounded-2xl transition-colors border border-charcoal/5 shadow-sm bg-white">
